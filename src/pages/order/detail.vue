...
<template>
  <s-layout title="订单详情" class="index-wrap" navbar="inner">
    ...
    <!-- 收货地址 -->
    <view v-if="state.orderInfo.receiverAreaId > 0" class="order-address-box">
    ...
    <view class="detail-goods" :style="[{ marginTop: state.orderInfo.receiverAreaId > 0 ? '0' : '-40rpx' }]">
      <!-- 订单信 -->
      <view v-for="item in state.orderInfo.items" :key="item.goods_id" class="order-list">
        <view class="order-card">
          ...
          >
            ...
            <template #priceSuffix>
              <button v-if="item.status_text" class="ss-reset-button tag-btn">
                {{ item.status_text }}
              </button>
            </template>
          </s-goods-item>
    ...
    <view class="notice-box">
      <view class="notice-box__content">
        ...
        <view v-if="state.orderInfo.payTime" class="notice-item">
    ...
    <view class="order-price-box">
      ...
      <!-- TODO 芋艿：优惠劵抵扣、积分抵扣 -->
      <view v-if="state.orderInfo.couponPrice > 0" class="notice-item ss-flex ss-row-between">
        <text class="title">优惠劵金额</text>
        <text class="detail">-¥{{ fen2yuan(state.orderInfo.couponPrice) }}</text>
      </view>
      <view v-if="state.orderInfo.discountPrice > 0" class="notice-item ss-flex ss-row-between">
        <text class="title">活动优惠</text>
        <text class="detail">¥{{ fen2yuan(state.orderInfo.discountPrice) }}</text>
      </view>
      <view v-if="state.orderInfo.vipPrice > 0" class="notice-item ss-flex ss-row-between">
      ...
      <view v-if="state.orderInfo.refundPrice > 0" class="notice-item all-rpice-item ss-flex ss-m-t-20">
    ...
    <su-fixed v-if="state.orderInfo.buttons?.length" bottom placeholder bg="bg-white">
...
import { fen2yuan, formatOrderStatus, formatOrderStatusDescription, handleOrderButtons } from '@/hooks/useGoods'
