<template>
  <s-layout title="确认订单">
    ...
    <view class="bg-white address-box ss-m-b-14 ss-r-b-10" @tap="onSelectAddress">
      <s-address-item :item="state.addressInfo" :has-border-bottom="false">
    ...
    <view class="bg-white total-card-box ss-p-20 ss-m-b-14 ss-r-10">
      <view class="total-box-content border-bottom">
        ...
        <!-- TODO 芋艿：接入积分 -->
        <view v-if="state.orderPayload.order_type === 'score'" class="order-item ss-flex ss-col-center ss-row-between">
        ...
        <view class="order-item ss-flex ss-col-center ss-row-between">
          ...
          <view class="ss-flex ss-col-center">
            <text class="item-value ss-m-r-24"> +￥{{ fen2yuan(state.orderInfo.price.deliveryPrice) }} </text>
          </view>
        </view>
        <!-- 优惠劵：只有 type = 0 普通订单（非拼团、秒杀、砍价），才可以使用优惠劵 -->
        <view v-if="state.orderInfo.type === 0" class="order-item ss-flex ss-col-center ss-row-between">
          <view class="item-title">优惠券</view>
          <view class="ss-flex ss-col-center" @tap="state.showCoupon = true">
            <text v-if="state.orderPayload.couponId > 0" class="item-value text-red">
              -￥{{ fen2yuan(state.orderInfo.price.couponPrice) }}
            </text>
            <text v-else class="item-value" :class="state.couponInfo.length > 0 ? 'text-red' : 'text-disabled'">
              {{ state.couponInfo.length > 0 ? state.couponInfo.length + ' 张可用' : '暂无可用优惠券' }}
            </text>
            <text class="_icon-forward item-icon" />
          </view>
        </view>
        <view v-if="state.orderInfo.price.discountPrice > 0" class="order-item ss-flex ss-col-center ss-row-between">
        ...
        <view v-if="state.orderInfo.price.vipPrice > 0" class="order-item ss-flex ss-col-center ss-row-between">
    ...
    <!-- 底部 -->
    <su-fixed bottom :opacity="false" bg="bg-white" placeholder :no-fixed="false" :index="200">
