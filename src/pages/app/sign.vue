...
<template>
  <s-layout title="签到有礼">
    ...
    <view v-else-if="!state.loading" class="sign-wrap">
      <!-- 签到日历 -->
      <view class="content-box calendar">
        ...
        >
          <view v-for="(item, index) in state.signConfigList" :key="index" class="item">
            <view
              :class="
        ...
        <view class="myDateTable">
          <view class="ss-flex ss-col-center ss-row-center sign-box ss-m-y-40">
            <button v-if="!state.signInfo.todaySignIn" class="ss-reset-button sign-btn" @tap="onSign">签到</button>
            <button v-else class="ss-reset-button already-btn" disabled>已签到</button>
          </view>
        </view>
      </view>
      ...
      <view class="bg-white ss-m-t-16 ss-p-t-30 ss-p-b-60 ss-p-x-40">
        ...
        <view class="activity-des"> 2、据说连续签到第 {{ state.maxDay }} 天可获得超额积分，一定要坚持签到哦~~~ </view>
      </view>
    </view>

    <!-- 签到结果弹窗 -->
    <su-popup :show="state.showModel" type="center" round="10" :is-mask-click="false">
