...
<template>
  <s-layout title="用户信息" class="set-userinfo-wrap">
    <uni-forms :model="state.model" :rules="state.rules" label-position="left" border class="form-box">
      <!-- 头像 -->
      ...
      <view class="bg-white ss-p-x-30">
        ...
        <uni-forms-item name="sex" label="性别">
          <view class="ss-flex ss-col-center ss-h-100">
            <radio-group class="ss-flex ss-col-center" @change="onChangeGender">
              <label v-for="item in sexRadioMap" :key="item.value" class="radio">
          ...
          >
            <template #right>
              <view class="ss-flex ss-col-center">
                <su-radio v-if="userInfo.verification?.mobile" :model-value="true" />
        ...
        <uni-forms-item name="password" label="登录密码" @tap="onSetPassword">
          <uni-easyinput
            ...
            :styles="{ disableColor: '#fff' }"
            disabled
            placeholder-style="color:#BBBBBB;font-size:28rpx;line-height:normal"
            :clearable="false"
          >
            <template #right>
              <view class="ss-flex ss-col-center">
                <su-radio v-if="userInfo.verification?.password" class="ss-flex" :model-value="true" />
                <button v-else class="ss-reset-button ss-flex ss-col-center ss-row-center">
    ...
    <view v-if="sheep.$platform.name !== 'H5'">
      ...
      <view class="account-list ss-flex ss-row-between">
        ...
        <view class="ss-flex ss-col-center">
          <view v-if="state.thirdInfo" class="info ss-flex ss-col-center">
