...
<template>
  <s-layout class="wallet-wrap" title="我的积分" navbar="inner">
    ...
    <!-- tab -->
    <su-sticky :custom-nav-height="sys_navBar">
      <!-- 统计 -->
      <view class="filter-box ss-p-x-30 ss-flex ss-col-center ss-row-between">
        <uni-datetime-picker v-model="state.date" type="daterange" :end="state.today" @change="onChangeTime">
      ...
      <su-tabs :list="tabMaps" :scrollable="false" :current="state.currentTab" @change="onChange"></su-tabs>
    </su-sticky>
    ...
    <view class="list-box">
      <view v-if="state.pagination.total > 0">
        ...
        >
          ...
          <view v-if="item.point > 0" class="add">+{{ item.point }}</view>
          <view v-else class="minus">{{ item.point }}</view>
        </view>
      </view>
