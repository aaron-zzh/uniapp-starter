...
<template>
  <view class="ss-goods-wrap">
    ...
    <view v-if="size === 'md'" class="md-goods-card ss-flex-col" :style="[elStyles]" @tap="onClick">
      ...
      <view :id="elId" class="md-goods-content ss-flex-col ss-row-around ss-p-b-20 ss-p-t-20 ss-p-x-16">
        <view
          v-if="goodsFields.title?.show || goodsFields.name?.show"
        ...
        <slot name="activity">
          <view v-if="data.promos?.length" class="tag-box ss-flex-wrap ss-flex ss-col-center">
            <view v-for="item in data.promos" :key="item.id" class="activity-tag ss-m-r-10 ss-m-t-16">
              {{ item.title }}
            </view>
          </view>
        </slot>
    ...
    <view v-if="size === 'lg'" class="lg-goods-card ss-flex ss-col-stretch" :style="[elStyles]" @tap="onClick">
        ...
        <view>
          <slot name="activity">
            <view v-if="data.promos?.length" class="tag-box ss-flex ss-col-center">
              <view v-for="item in data.promos" :key="item.id" class="activity-tag ss-m-r-10">
                {{ item.title }}
              </view>
            </view>
          </slot>
      ...
      <slot name="cart">
        <view v-if="buttonShow" class="buy-box ss-flex ss-col-center ss-row-center"> 去购买 </view>
      </slot>
    </view>
...
import { fen2yuan, formatSales } from '@/hooks/useGoods'
import { formatStock } from '@/hooks/useGoods'
