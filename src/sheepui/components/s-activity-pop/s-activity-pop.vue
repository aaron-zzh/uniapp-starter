<!-- 商品信息：满减送等营销活动的弹窗 -->
<template>
  <su-popup :show="show" type="bottom" round="20" show-close @close="emits('close')">
    <view class="model-box">
      <view class="title ss-m-t-16 ss-m-l-20 ss-flex">营销活动</view>
      <scroll-view class="model-content ss-m-t-50" scroll-y :scroll-with-animation="false" :enable-back-to-top="true">
        <view v-for="item in state.activityInfo" :key="item.id">
          <view class="ss-flex ss-col-top ss-m-b-40" @tap="onGoodsList(item)">
            ...
            <view class="ss-m-l-20 model-content-title ss-flex-1">
              <view v-for="rule in state.activityMap[item.id]?.rules" :key="rule" class="ss-m-b-24">
...
import { formatRewardActivityRule } from '@/hooks/useGoods'
