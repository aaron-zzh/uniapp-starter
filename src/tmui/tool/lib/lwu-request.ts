...
export class Http {
   ...
   */
  private handleError(code: number, message = ''): void {
    // 调用错误状态码处理程序
  ...
  public request(
    ...
    return new Promise((resolve, reject) => {
      ...
      setToken().then((getToken) => {
        ...
        if (options?.task_id) {
          // 当前请求存入缓存
          const tasks: UniApp.RequestTask[] = []
