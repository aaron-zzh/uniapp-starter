<template>
  <tm-translate v-if="showBody" ref="bodyani" :reverse="reani" :name="hidnAniName" :auto-play="false" @end="hide">
    <tm-sheet
      ...
      <view class="relative bdcld flex flex-row flex-row-top-start">
            ...
            >
              <view class="flex-12 flex flex-row flex-row-top-start">
                <view v-if="item['icon']" class="pr-32" :style="{ height: '36rpx' }">
                  <tm-icon :font-size="32" :line-height="0" :name="item['icon']"></tm-icon>
                </view>
                <view style="width: 0px" class="flex-12 flex flex-col">
                  ...
                  <view v-if="item['content']" class="flex-12 overflow">
                    <tm-text
                      ...
                      @click="emits('click', item, index)"
                    ></tm-text>
                    <tm-text v-if="len > 1 && props.showDot" _class="pt-24" :label="`${activeIndex + 1}/${len}`">
                    </tm-text>
                  </view>
                </view>
              </view>
        ...
        <slot name="right">
          <view v-if="closable" class="flex flex-row flex-row-top-end">
            <tm-icon :font-size="32" :line-height="0" name="tmicon-times-circle-fill" @click="close"></tm-icon>
          </view>
        </slot>
      </view>
    </tm-sheet>
