    ...
    <template #default>
      <view class="pa-24">
        <view class="flex flex-row flex-row-center-start">
          <tm-image v-if="nowItemInfo" :preview="true" :width="120" :height="120" :src="nowItemInfo.img"></tm-image>
          <view class="pl-24">
            <view class="flex flex-row flex-row-center-start">
        ...
        <view class="mb-24">
          <view class="flex flex-row flex-row-center-between mb-24" @click="addNumberClick">
        ...
        <view v-if="_list">
          <view v-for="(item, index) in _list.data" :key="index" class="">
            <tm-text _class="mb-24" :font-size="28" :label="item.title"></tm-text>
            <tm-radio-group v-if="item?.children" v-model="nowSelected[index]" direction="row">
              <tm-radio
    ...
    <template #foot>
      <view class="mb-20 px-24">
        <view style="height: 40rpx" class="flex flex-row flex-row-center-center">
          ...
          <tm-text
            v-if="nowInputNumber > maxCount && nowInputNumber && nowItemInfo"
            :color="props.color"
            label="库存不足"
          ></tm-text>
        </view>
