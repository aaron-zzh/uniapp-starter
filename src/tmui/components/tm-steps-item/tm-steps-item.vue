<template>
  ...
  >
    <!-- 线 -->
    <view
      class="flex-1 flex-row flex-row-center-between absolute"
      :style="{ width: _width, top: nodeSize / 2 + 'rpx' }"
    >
    ...
    >
      ...
      >
        <tm-text
          ...
          :label="_cureent + 1"
        ></tm-text>
        <tm-icon v-if="status && _isActive && _typeModel != 'dot'" :font-size="22" :name="status.icon"></tm-icon>
        <tm-icon v-if="!status && _icon && _typeModel != 'dot'" :font-size="22" :name="_icon"> </tm-icon>
      </tm-sheet>
      <view :event-penetration-enabled="true" style="width: 120rpx" class="flex flex-col flex-col-center-center mt-12">
        <slot name="default">
          <tm-text
            v-if="props.title"
            :user-interaction-enabled="false"
