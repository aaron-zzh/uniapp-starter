  ...
  <view class="flex flex-row">
    ...
    >
      <scroll-view
        ...
        scroll-y
        :style="{ height: _height + 'rpx', width: _sliderMenuWidth + 'rpx' }"
      >
        <tm-sheet
        ...
        >
          <tm-sheet
            ...
            :paren-class="
              (_active - 1 === index ? 'round-br-6' : '') + ' ' + (_active + 1 === index ? 'round-tr-6' : '')
            "
          ...
          >
            <view
              ...
              class="absolute l-6"
              :style="{ top: (_sliderMenuItemHeight - 30) / 2 + 'rpx' }"
            >
              <tm-sheet
                v-if="_active == index"
                :round="2"
            ...
            <tm-badge v-if="!item.icon" :user-interaction-enabled="false" color="red" :count="item.dotCount">
              <view
                class="flex flex-col flex-col-center-center py-10"
                :style="{ width: `${_sliderMenuWidth - 28}rpx` }"
              >
                <tm-text
            ...
            <tm-badge v-if="item.icon" :user-interaction-enabled="false" color="red" :count="item.dotCount">
              <view
                class="flex flex-row flex-row-center-center py-10"
                :style="{ width: `${_sliderMenuWidth - 28}rpx` }"
              >
                <tm-icon
                  v-if="item.icon"
                  ...
                  :name="item.icon"
                ></tm-icon>
                <view :user-interaction-enabled="false" class="px-10">
                  <tm-text
                    :_class="'text-align-center ' + (_active == index ? 'text-weight-b' : '')"
                    :color="_active == index ? props.menuActiveFontColor : props.menuFontColor"
                    :font-size="props.fontSize"
...
async function actionLoad(type: 'pull' | 'bottom' | 'menu', item: listItem | null = null, index = NaN) {
  let fun = props.load
  let p: any = true
