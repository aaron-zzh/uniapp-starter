...
export const color = {
  isValidColor(color: string): boolean {
    ...
    const rgbRegExp =
      /^rgb\((\d{1,2}|1\d{1,2}|2[0-4]\d|25[0-5]),\s*(\d{1,2}|1\d{1,2}|2[0-4]\d|25[0-5]),\s*(\d{1,2}|1\d{1,2}|2[0-4]\d|25[0-5])\)$/
    const rgbaRegExp =
  ...
  rgbaToRgba(rgba1: [number, number, number, number], rgba2: [number, number, number, number], blv = 0) {
    const result = subtract(rgba1, rgba2)
    if (result[0] <= 0) {
