<template>
  <view
    ...
    @mouseleave="touchEnd"
  >
    <view v-if="!url" class="flex flex-center py-24"
      ><text style="font-size: 24rpx; color: white">请先选择图片</text>
    </view>
    <!-- #ifndef APP-NVUE -->
    <canvas
      id="imgid"
      ...
      :style="[
        {
          ...
          transform: url
            ? `translate(${imgPos.x}px,${imgPos.y}px) scale(${imgPos.scale},${imgPos.scale})`
    ...
    <view class="absolute l-0 b-0 zIndex-2 black" :style="{ width: `${view_width}px`, height: `${80}px` }">
      ...
      <view class="flex-row flex flex-row-center-between">
        ...
        <!-- 取消 -->
        <view class="flex-1 flex flex-center" @click="emits('cance')">
          <tm-text :user-interaction-enabled="false" color="white" label="取消"></tm-text>
        </view>
        <!-- 选择图片 -->
        <view class="flex-1 flex flex-center" @click="chooseImg">
          <tm-text :user-interaction-enabled="false" color="white" label="选择图片"></tm-text>
        </view>
        <!-- 确认 -->
        <view class="flex-1 flex flex-center" @click="pushImgToCanvas">
          <tm-text :user-interaction-enabled="false" color="white" label="确认"></tm-text>
        </view>
      </view>
    </view>
  </view>
